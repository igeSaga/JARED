...
#Fensterfläche
Der Fensteranteil wird als Prozent der Wandfläche für unterschiedliche Bauperioden geschätzt, basierend auf einer Lookup-Table von <Thomas Schluck>. Die Fensterfläche wird sowohl in Prozent als auch als absolute Fläche angegeben.

##Wandfläche berechnen
Die Wandfläche wird aus dem Umfang des Gebäudes und der Stockwerkzahl gerechnet. Die Stockwerkhöhe von Wohngebäuden wird auf 3m festgelegt, bei nicht Wohngebäuden sind es 5m.
...
Achtung: Die Berechnung stimmt nur für Gebäude mit relativ simpler Geometrie. Die Berechnung von Länge und Breite, und daraus abgeleitet die Ausrichtung, basieren auf der Convex-Hull der Gebäude. Für komplexe Formen, z.B. U-förmig oder gekrümmt, stimmt die Berechnung nicht. Um diesem Fehler beachtung zu schenken, wird ein Attribut hinzugefügt, welches die Ratio zwischen der Convex-Hull Fläche und der original Fläche des Gebäudes abbildet (ConvexHullArea/OrigArea, ist nie grösser als 1).
...
for(ind in 1:nrow(rolle_bld_out)){
  ...
  # plot(SpatialPolygons(list(rolle_bld_out@polygons[[ind]])))
  # plot(lm[[6]],add=T)
  # plot(line.mj.int, col="red",add=T)
