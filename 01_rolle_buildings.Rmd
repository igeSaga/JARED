---
title: "01 - Gebäudedaten Einlesen, Aufbereiten und Bereinigen"
...
#working directory
#must be changed if working on Windows
...
In diesem Skript werden alle relevanten Gebäudedaten eingelesen und miteinander verschnitten. Die Gebäudegeometrien werden aus der amtlichen Vermessung und von Open Steet Map (OSM) bezogen. Gebäudeinformationen zu Heizung, Alter, etc. werden aus den GWR Punkten hinzugefügt.
...
```{r readerGwr}
##csv lesen
rolle_gwr<-read.csv(paste(dataPath,"02_GWR/Rolle_GEB-02.csv",sep=""),sep = ";",encoding = "Latin-1")
summary(rolle_gwr$GWW2)
...
rolle_gwr<-rolle_gwr[,c("EGID","GSTAT","GKAT","GKLAS","GAREA","GASTW","GBAUJ","GANZWHG","GABBJ","GHEIZ","GENHZ","GENWW","GWWV")]
...
#filter/rename attributes
ms_rolle_gwr<-ms_rolle_gwr[,c("EGID","STATUT_BAT","CATEGORIE","CLASSE","SURFACE","NB_NIV","CONS_ANNEE","NB_LOG","DEMOL_ANNE","CHAUFF_SYS","CHAUFF_AGT","EAU_CHD_AG","EAU_CHD_IN")]
names(ms_rolle_gwr)<-c("EGID","GSTAT","GKAT","GKLAS","GAREA","GASTW","GBAUJ","GANZWHG","GABBJ","GHEIZ","GENHZ","GENWW","GWWV")
...
rolle_bld_gwr_out<-rolle_bld_gwr[,c("EGID","GSTAT","GKAT","GKLAS","GAREA","GASTW","GBAUJ","GANZWHG","GABBJ","GHEIZ","GENHZ","GENWW","GWWV")]
rolle_bld_gwr_mult_out<-rolle_bld_gwr_mult[,c("EGID","GSTAT","GKAT","GKLAS","GAREA","GASTW","GBAUJ","GANZWHG","GABBJ","GHEIZ","GENHZ","GENWW","GWWV")]
rolle_osm_gwr_out<-rolle_osm_gwr[,c("EGID","GSTAT","GKAT","GKLAS","GAREA","GASTW","GBAUJ","GANZWHG","GABBJ","GHEIZ","GENHZ","GENWW","GWWV")]
...
rolle_schloss@data[1,]<-c("9999999",rep(NA,3),round(rolle_schloss@polygons[[1]]@area,0),rep(NA,9))
rolle_schloss$source<-"manual"
rolle_schloss$GWWV<-NA
...
for(i in 1:nrow(rolle_av)){
  rolle_av@data[i,]<-c(paste("9999_ID",as.character(rolle_av_temp$ID[i]),sep=""),rep(NA,3),round(rolle_av@polygons[[i]]@area,0),rep(NA,9))
}
...
wd<-getwd()
htmlwidgets::saveWidget(m, file=paste(wd,"/output/01_output/map/01_map.html",sep=""),selfcontained = T)

#buildings speichern
save(rolle_bld_out,file="output/01_output/geom/01_rolle_bld_out.Rdata")
save(rolle_build_av_excl,file="output/01_output/geom/01_rolle_bld_av_notGwr.Rdata")

writeOGR(rolle_bld_out, "output/01_output/geom", "01_rolle_bld_out", driver="ESRI Shapefile",overwrite_layer = T)
writeOGR(rolle_bld_out.wgs, "output/01_output/geom", "01_rolle_bld_out_wgs", driver="ESRI Shapefile",overwrite_layer = T)
writeOGR(rolle_build_av_excl.wgs, "output/01_output/geom", "01_rolle_bld_av_notGwr_wgs", driver="ESRI Shapefile",overwrite_layer = T)

#abbildungen speichern
ggsave(p.all,filename = "output/01_output/viz/01_buildingPlots.png",dpi = 500,width = 22,height = 15)
```
