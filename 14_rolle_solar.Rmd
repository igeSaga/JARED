...
test<-test%>%
  group_by(iid_dach)%>%
  summarise(area_dach_tot=sum(area_inter))
...
dach_bld_inter.aggreg<-dach_bld_inter.tbl%>%
  group_by(EGID)%>%
  summarise(pvPotentialJahr=sum(pvJahr_dach_proz, na.rm=T)/6000)
...
writeOGR(dach_bld_inter.sp, "output/14_output/geom", "dach_bld_inter", driver="ESRI Shapefile",overwrite_layer = T)
```
